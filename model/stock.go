...
type Stock struct {
	gorm.Model
	Name             string `gorm:"type:varchar(50);not null;comment:股票名称;index:unique-name-date,unique;priority:1"`
	Url              string `gorm:"type:varchar(1000);not null;comment:股票链接;"`
	OnListDate       string `gorm:"type:varchar(50);not null;comment:上榜日期;index:unique-name-date,unique;priority:2"`
